...
import { chunkedBatchModify } from './helpers.js';
...
const log = logger.child('api:filters');
...
export async function updateFilter(
  ...
  if (query !== '' && hasLabelChanges) {
    ...
    if (messages.length > 0) {
      ...
      const result = await chunkedBatchModify(client, messageIds, addIds, removeIds);
      retroactive = {
        ...result,
        message:
          result.failed.length === 0
            ? `Retroactively modified ${result.modified} message(s).`
            : `Retroactively modified ${result.modified} of ${messageIds.length} messages. ${result.failed.length} failed.`,
      };
    }
  }
